...
class Busqueda(models.Model):
    ...
    total_encontradas = models.IntegerField('Encontradas', default=0)
    nuevas = models.IntegerField('Nuevas', default=0)
    existentes = models.IntegerField('Ya registradas', default=0)
    created = models.DateTimeField('Fecha de búsqueda', auto_now_add=True)
    ...
    def __str__(self):
        return f"{self.keyword} ({self.total_encontradas} ofertas)"


class OfertaEmpleo(models.Model):
    api_id = models.CharField('ID API', max_length=250, unique=True, db_index=True)
    ...
    posted_date = models.CharField('Fecha de publicación', max_length=50, blank=True)
    ...
    busquedas = models.ManyToManyField(Busqueda, blank=True, related_name='ofertas')
